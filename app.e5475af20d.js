...
// A SINGLE-layer entry has only the compound's own vigraha with NO member layers to step through — it is a
// shallow (non-deep) samāsa: nothing to PEEL in the node, so it is dropped here. Its vigraha-vākya still
// surfaces in the reading-view hover (the gloss bakes SAMASA_PEEL directly). Deep peels (≥2 layers) stay.
// (Harsha, 2026-09-19: प्रकारान्तर etc. — single-element peels → gloss yes, samāsa-peel node no.)
function samrHasCompound(key) { const L = samrPeelLayers(key); return Array.isArray(L) && L.length >= 2 && L.some(l => isCompoundSamasaType(l && l.type)); }
...
// Generalised case-fidelity (2026-09-25): the ṣaṣṭhī rule below was the only honoured override, so 150 other
// case questions (99 कर्म, 52 कर्ता) graded AGAINST the wordCode displayed under the word. When every present
// role word stands in ONE case other than the voice default, accept it too and explain. Grammatical cells make
// the text's case the headline answer (passive + द्वितीया secondary object 1.4.51 अकथितं च; सति-सप्तमी agent
// 2.3.37; motion-goal चतुर्थी 2.3.12); other cells (usually an analysis slip or a syncretic form) keep the
// voice default as headline — but the displayed case is ALWAYS accepted, so the text's own form is never wrong.
function uniformActualCase(indices, sentence) {
  const acts = [...new Set((indices || []).map(i => actualVibhakti(sentence, i)).filter(Boolean))];
  return acts.length === 1 ? acts[0] : null;
}
function caseMismatchSpec(role, voice, base, shown, words) {
  const w = `<b>${esc(words)}</b>`;
  if (role === 'karma' && voice === 'कर्मणि' && shown === 'द्वितीया')
    return { correct: 'द्वितीया', tip: `${w} stands in <b>द्वितीया</b> although the verb is कर्मणि. In the passive only the principal object is promoted to प्रथमा; a secondary object (अकथित कर्म — 1.4.51 अकथितं च) or an object governed by an upasarga keeps द्वितीया. The plain rule (प्रथमा) is also accepted.` };
  if (role === 'karta' && shown === 'सप्तमी')
    return { correct: 'सप्तमी', tip: `${w} stands in <b>सप्तमी</b>: it is the subject of a सति-सप्तमी (locative absolute — 2.3.37 यस्य च भावेन भावलक्षणम्), which puts both the participle and its subject in सप्तमी. The voice default (${base}) is also accepted.` };
  if (role === 'karma' && shown === 'चतुर्थी')
    return { correct: 'चतुर्थी', tip: `${w} stands in <b>चतुर्थी</b>: the goal of a verb of motion may take चतुर्थी instead of द्वितीया (2.3.12 गत्यर्थकर्मणि द्वितीयाचतुर्थ्यौ चेष्टायामनध्वनि). द्वितीया is also accepted.` };
  return { correct: base, tip: `The voice rule predicts <b>${base}</b> for the ${role === 'karta' ? 'कर्ता' : 'कर्म'}, but ${w} is coded <b>${shown}</b> here — both are accepted. (Such a mismatch usually means the form is identical in both cases, or the word plays a slightly different role than a plain ${role === 'karta' ? 'कर्ता' : 'कर्म'}.)` };
}
...
function kartaCaseOptions(c, sentence) {
  ...
  if (sIdx == null) {
    const shown = uniformActualCase(c.karta, sentence);
    if (shown && shown !== base) {
      const m = caseMismatchSpec('karta', c.voice, base, shown, (c.karta || []).map(i => sentence.words[i]).join(' / '));
      const options = [...new Set(['प्रथमा', 'तृतीया', ...(c.karmaGovernorIsKrdanta ? ['षष्ठी'] : []), shown])];
      return { correct: m.correct, options, accept: [...new Set([m.correct, base, shown])], tip: m.tip };
    }
  }
...
function karmaCaseOptions(c, sentence) {
  ...
  if (sIdx == null) {
    const shown = uniformActualCase(c.karma, sentence);
    if (shown && shown !== base) {
      const m = caseMismatchSpec('karma', c.voice, base, shown, (c.karma || []).map(i => sentence.words[i]).join(' / '));
      const options = [...new Set(['द्वितीया', 'प्रथमा', ...(c.karmaGovernorIsKrdanta ? ['षष्ठी'] : []), shown])];
      return { correct: m.correct, options, accept: [...new Set([m.correct, base, shown])], tip: m.tip };
    }
  }
  const correct = sIdx != null ? 'षष्ठी' : base;
...
const CLAUSE_ROLE_LABEL = {
  ...
  hetu: ['हेतु', 'the cause/motive — “because of what?” (तृतीया by 2.3.23 हेतौ; पञ्चमी by 2.3.24/25)'],
...
// CLAUSE-SCOPED role words (2026-09-25). A cluster's कारक words can sit in ANOTHER gold clause (AB 10 यथा आकाशः is
// the upamāna of भाति in clause 1 but a member of clause 0; isha 1.1 जगत्याम्) — asking them in this clause's trace
// taught a boundary the next step contradicted and double-asked them in their own clause's mop-up (257 verses).
// Every trace step / guard / traced-set reads through these, so a cross-clause word is simply left to its OWN
// clause's mop-up. Same rule build_anvaya.js already applies. Primary role must be present IN the clause for the
// समुच्चय partners to be asked with it (mirrors the old guard).
function clauseWordSet(sentence, clauseIdx) { return new Set((((sentence.clauses || [])[clauseIdx]) || {}).words || []); }
function clauseRoleIn(sentence, clauseIdx, cluster, role) {
  if (!cluster) return [];
  const cw = clauseWordSet(sentence, clauseIdx);
  return roleIndices(cluster, role).filter(i => cw.has(i));
}
function clauseCoreIn(sentence, clauseIdx, cluster, which /* 'karta'|'karma' */) {
  if (!cluster) return [];
  const cw = clauseWordSet(sentence, clauseIdx);
  const prim = (cluster[which] || []).filter(i => cw.has(i));
  if (!prim.length) return [];
  const partner = which === 'karta' ? 'samuccayaKarta' : 'samuccayaKarma';
  return [...prim, ...(cluster[partner] || []).filter(i => cw.has(i))];
}
...
function clauseTracedIndices(sentence, clauseIdx) {
  ...
  if (hc) {
    for (const i of clauseCoreIn(sentence, clauseIdx, hc, 'karta')) s.add(i);
    for (const i of clauseCoreIn(sentence, clauseIdx, hc, 'karma')) s.add(i);
    for (const role of CLAUSE_PERIPHERAL_ROLES) for (const i of clauseRoleIn(sentence, clauseIdx, hc, role)) s.add(i);
  }
  ...
  if (clauseNucleiCount(sentence) >= 2) {
    for (const { cluster: gc } of clauseGerundClusters(sentence, clauseIdx)) {
      ...
      for (const i of clauseCoreIn(sentence, clauseIdx, gc, 'karta')) s.add(i);
      for (const i of clauseCoreIn(sentence, clauseIdx, gc, 'karma')) s.add(i);
      for (const role of CLAUSE_PERIPHERAL_ROLES) for (const i of clauseRoleIn(sentence, clauseIdx, gc, role)) s.add(i);
    }
  }
  return s;
}
...
function vibhaktiOverlayCallout(sentence, expectedSet) {
  ...
  for (const i of expectedSet) {
    const ov = wordVibhakti(sentence, i);
    if (!ov || !ov.num || !ov.type || ov.type === 'कारक' || VIBHAKTI_DEFAULT_SUTRAS.has(ov.num)) continue;
...
function buildClauseSteps(sentence) {
  ...
  if (clauseStyle === 'socratic') {
    ...
    clauses.forEach((cl, i) => {
      ...
      if (clauseCoreIn(sentence, i, hc, 'karma').length) { steps.push({ type: 'clauseKarmaCase', clauseIdx: i }); steps.push({ type: 'clauseKarma', clauseIdx: i }); }
      // (2b) the PRESENT agent (an अनुक्त/elided agent has no word to click → asked in the supply phase)
      if (clauseCoreIn(sentence, i, hc, 'karta').length) steps.push({ type: 'clauseKartaTrace', clauseIdx: i });
      // (2c) present peripheral kārakas — each teaches the same-case-vs-कारक discrimination
      for (const role of CLAUSE_PERIPHERAL_ROLES) if (clauseRoleIn(sentence, i, hc, role).length) steps.push({ type: 'clausePeripheral', clauseIdx: i, role });
      ...
      if (nucleiCount >= 2) {
        for (const { cluster: gc, clusterIdx: gci } of clauseGerundClusters(sentence, i)) {
          ...
          if (clauseCoreIn(sentence, i, gc, 'karma').length) { steps.push({ type: 'clauseGerundKarmaCase', clauseIdx: i, clusterIdx: gci }); steps.push({ type: 'clauseGerundKarma', clauseIdx: i, clusterIdx: gci }); }
          if (clauseCoreIn(sentence, i, gc, 'karta').length) steps.push({ type: 'clauseGerundKarta', clauseIdx: i, clusterIdx: gci });   // an absolutive rarely has its own कर्ता, but क्त/शतृ participles can
          for (const role of CLAUSE_PERIPHERAL_ROLES) if (clauseRoleIn(sentence, i, gc, role).length) steps.push({ type: 'clauseGerundPeripheral', clauseIdx: i, clusterIdx: gci, role });
        }
      }
  ...
  if (clauseStyle !== 'socratic') clauses.forEach((cl, i) => {
    const cluster = clusterForClauseHead(sentence, cl);
    if (cluster && cluster.voice && clauseCoreIn(sentence, i, cluster, 'karma').length) {
  ...
  clauses.forEach((cl, i) => {
    const el = elidedString(cl);
    if (!el || seenElided.has(el) || !clauseElidedOptions(sentence, i)) return;
    if (el === stripParens(normalizeAgent(cl.anuktaKarta))) return;   // the supplied word IS the agent — already asked as कर्ता, don't re-ask as an elided verb
    seenElided.add(el);
...
function expectedSetForStep(sentence, step) {
  ...
  if (step.type === 'clausePeripheral') return new Set(clauseRoleIn(sentence, step.clauseIdx, clusterForClauseHead(sentence, (sentence.clauses || [])[step.clauseIdx]), step.role));
  if (step.type === 'clauseGerundKarma') return new Set(clauseCoreIn(sentence, step.clauseIdx, sentence.clusters[step.clusterIdx], 'karma'));
  if (step.type === 'clauseGerundKarta') return new Set(clauseCoreIn(sentence, step.clauseIdx, sentence.clusters[step.clusterIdx], 'karta'));
  if (step.type === 'clauseGerundPeripheral') return new Set(clauseRoleIn(sentence, step.clauseIdx, sentence.clusters[step.clusterIdx], step.role));
...
function coreArgIndices(c, side) {
  if (side === 'karta') {
    ...
    // …EXCEPT a कृदन्त predicate head that has its own explicit कर्ता: katha 1.2.9 प्रोक्ता (कर्ता अन्येन) is the
    // PREDICATE, not the subject, so the कर्ता question must not accept प्रोक्ता itself. A noun head (brihad 4.4.8
    // पन्थाः, mundaka 1.2.4 जिह्वाः) IS the subject and stays accepted. (2026-09-25)
    const krtPredicateWithAgent = c.karmaGovernorIsKrdanta && (c.karta || []).length;
    if (c.subjectIsHead && !krtPredicateWithAgent && !base.includes(c.governorWordIndex)) return [c.governorWordIndex, ...base];
    return base;
  }
...
// Display canonicalisation of the model's spelling variants of ONE type label (prādi-tatpuruṣa written with and
// without the hyphen, लोपि/लोपी, पंचमी/पञ्चमी, a stray-virāma तृ्तीया, …) so the same type never shows two ways.
const SAMASA_TYPE_CANON = [
  [/प्रादितत्पुरुष/g, 'प्रादि-तत्पुरुष'], [/इतरेतरद्वन्द्व/g, 'इतरेतर-द्वन्द्व'], [/मध्यमपदलोपी/g, 'मध्यमपदलोपि'],
  [/मध्यमपदलोपि तत्पुरुष/g, 'मध्यमपदलोपि-तत्पुरुष'], [/मयूर्व्यंसकादि/g, 'मयूरव्यंसकादि'], [/मयूरव्यंसकादि तत्पुरुष/g, 'मयूरव्यंसकादि-तत्पुरुष'],
  [/पंचमी/g, 'पञ्चमी'], [/द्वितीय-तत्पुरुष/g, 'द्वितीया-तत्पुरुष'], [/तृ्तीया/g, 'तृतीया'], [/तुल्ययोगे बहुव्रीहिः?/g, 'तुल्ययोग-बहुव्रीहि'],
  [/अवधारण-पूर्वपद|अवधारणापूर्वपद/g, 'अवधारणा-पूर्वपद'],
];
function canonSamasaType(t) { let x = String(t || ''); for (const [re, to] of SAMASA_TYPE_CANON) x = x.replace(re, to); return x; }
function samasaTypeOptions(correct) {
  correct = canonSamasaType(correct);
  // Never offer a label the answer CONTAINS or descends from — it isn't wrong, it's a coarser name for the same
  // compound (2026-09-25: नञ्-बहुव्रीहि ×336 / प्रादि-बहुव्रीहि ×172 were offered "बहुव्रीहि"; द्विगु offered कर्मधारय;
  // a slash label "अव्ययीभाव / कर्मधारय (वीप्सा)" offered either half).
  const parts = correct.split('/').map(p => p.trim()).filter(Boolean);
  const isAncestor = t => parts.some(p => (parts.length > 1 || p !== t) && p.includes(t))   // a slash label's own halves count too
    || (/द्विगु/.test(correct) && /कर्मधारय/.test(t))      // द्विगु is a kind of कर्मधारय (2.1.52 संख्यापूर्वो द्विगुः)
    || (correct === 'कर्मधारय' && t === 'द्विगु');          // … so a plain कर्मधारय may be one
  let pool = SAMASA_TYPE_POOL.filter(t => t !== correct && !isAncestor(t));
  ...
  // vary the distractors among the most COMMON types (not always the same first three — a rarer one then read as
  // the odd one out), still keeping them common ([[feedback_quiz_distractor_rarity]]).
  const distract = seedRotate(pool.slice(0, 6), correct).slice(0, 3);
...
// Same idea for the other copulas the gold supplies (भवति ×258, स्यात् ×47): offer agreement-wrong forms of the
// SAME verb, never a synonym from another copula (अस्ति for भवति is arguably also correct). (2026-09-25)
const COPULA_PARADIGMS = [
  ASTI_PARADIGM,
  ['भवामि', 'भवसि', 'भवति', 'भवावः', 'भवथः', 'भवतः', 'भवामः', 'भवथ', 'भवन्ति'],
  ['स्याम्', 'स्याः', 'स्यात्', 'स्याव', 'स्यातम्', 'स्याताम्', 'स्याम', 'स्यात', 'स्युः'],
  ['आसम्', 'आसीः', 'आसीत्', 'आस्व', 'आस्तम्', 'आस्ताम्', 'आस्म', 'आस्त', 'आसन्'],
  ['अस्तु', 'स्ताम्', 'सन्तु'],
];
const COPULA_FORMS = new Set(COPULA_PARADIGMS.flat());
// the gold sometimes brackets a supplied word "(अस्ति)" — the bare form is the answer (else the paradigm check,
...
function clauseElidedOptions(sentence, clauseIdx) {
  ...
  let pool;
  const para = COPULA_PARADIGMS.find(P => P.includes(correct));
  if (para) {
    pool = para.filter(f => f !== correct);   // agreement-wrong forms of the SAME verb
  } else {
    // siblings' own supplied words — but never a bare copula when the answer is a multi-word/other supply
    // that CONTAINS a copula (VC 2 "दुर्लभम् अस्ति" vs "अस्ति"): that's a near-duplicate, not a wrong answer.
    const sibs = [];
    (sentence.clauses || []).forEach(x => { const e = elidedString(x); if (e && e !== correct && !sibs.includes(e)) sibs.push(e); });
    const nearDup = e => COPULA_FORMS.has(e) && correct.split(/\s+/).some(w => COPULA_FORMS.has(w));
    pool = [...new Set(sibs.filter(e => !nearDup(e)))];
    if (!correct.split(/\s+/).some(w => COPULA_FORMS.has(w))) pool = [...new Set([...pool, 'अस्ति', 'सन्ति'].filter(e => e !== correct))];
  }
...
function samasaTip(L) {
  let s = `<b>${esc(L.c)}</b> = ${esc(L.vigraha)} — <b>${esc(canonSamasaType(L.type))}</b>`;
...
function clauseElidedTip(cl) {
  let s = `The words <b>${esc(elidedString(cl))}</b> are supplied by <b>अध्याहार</b> — understood here though left unstated.`;
...
// The PLAIN default sūtra of each case. The overlay's `type` field is inconsistent (2.3.50 typed "शेष" 3,015×,
// 2.3.28 typed "विशेष" 961×), so gating on type let the "not the plain default" MCQ ask the default ~2,370×.
// Decide by sūtra NUMBER instead. (2026-09-25)
const VIBHAKTI_DEFAULT_SUTRAS = new Set(['2.3.46', '2.3.2', '2.3.18', '2.3.13', '2.3.28', '2.3.50', '2.3.36', '2.3.47']);
function vibhaktiVidhanaEligible(sentence, i) {
  const e = (sentence._vibhakti || {})[i];
  if (!e || !e.num || !e.type || e.type === 'कारक' || !e.vibhakti || VIBHAKTI_DEFAULT_SUTRAS.has(e.num)) return false;
  const pool = (VIBHAKTI_CASE_SUTRAS[e.vibhakti] || []).filter(n => n !== e.num);
  return pool.length >= 1 && !!VIBHAKTI_SUTRA_TEXT[e.num];
}

// सकर्मक/अकर्मक valence, graded against what the TEXT shows (2026-09-25). The dhātupāṭha transitivity is the
// ROOT's; a causative (णिच्) or an upasarga can make an अकर्मक root take a कर्म — aitareya 1.1.3 अमूर्छयत्
// (पुरुषम्), AB 67 भासयते. Answering अकर्मक there and then asking for the कर्म contradicted itself (501 steps).
// So: a कर्म present in the clause ⇒ सकर्मक (with a note when the root is listed अकर्मक); no कर्म present ⇒ the
// root's valence, and a सकर्मक head without one gets "its कर्म is unexpressed here" (1,985 such heads).
function valenceSpec(head, transitivity, hasKarma, isGerund) {
  const rootAkarmaka = /अकर्मक/.test(transitivity || '');
  const correct = (hasKarma || !rootAkarmaka) ? 'सकर्मक' : 'अकर्मक';
  const h = `<b>${esc(head)}</b>`;
  let tip;
  if (correct === 'सकर्मक' && rootAkarmaka)
    tip = `${h} is used <b>सकर्मक</b> here — it has a कर्म in this ${isGerund ? 'frame' : 'clause'}. (Its root is listed अकर्मक; a causative/णिच् form or an upasarga lets it take an object.) Find that कर्म next.`;
  else if (correct === 'सकर्मक' && hasKarma)
    tip = isGerund ? `${h} is <b>सकर्मक</b> (transitive) — it governs its own कर्म, distinct from the finite verb's. Find that कर्म next.`
      : `${h} is <b>सकर्मक</b> (transitive) — its action reaches out to a कर्म. So look for the word it acts upon: that कर्म (and its vibhakti) draws this clause's boundary.`;
  else if (correct === 'सकर्मक')
    tip = `${h} is <b>सकर्मक</b> (transitive) by nature — but its कर्म is <b>not expressed</b> in this ${isGerund ? 'frame' : 'clause'} (understood from context), so there is no कर्म word to find here.`;
  else
    tip = isGerund ? `${h} is <b>अकर्मक</b> (intransitive) — it takes no कर्म of its own.`
      : `${h} is <b>अकर्मक</b> (intransitive) — it takes no कर्म. Its कर्ता alone completes the action; a द्वितीया word here would belong to some other verb, not to ${esc(head)}.`;
  return { correct, options: ['सकर्मक', 'अकर्मक'], explainHtml: tip };
}
function tutorialMcqSpec(step, sentence) {
  ...
  if (step.type === 'clauseValence') {
    ...
    const r = valenceSpec(head, hc.transitivity, clauseCoreIn(sentence, step.clauseIdx, hc, 'karma').length > 0, false);
    return { ...r, highlight: new Set(cl.headWordIndex != null ? [cl.headWordIndex] : []) };
  }
  if (step.type === 'clauseGerundAttach') {
    const cl = sentence.clauses[step.clauseIdx];
    const gc = sentence.clusters[step.clusterIdx] || {};
    const gw = gc.governorWord || '';
    // label each nucleus by its head word; when the SAME verb heads two clauses (brihad 1.2.5 समभवत् ×2) add the
    // clause number, else the options collapse into duplicates / a single button (207 + 10 steps). (2026-09-25)
    ...
    const heads = (sentence.clauses || []).map((_, idx) => headOf(idx));
    const labelOf = idx => heads.filter(h => h === heads[idx]).length > 1 ? `${heads[idx]} (वाक्य ${idx + 1})` : heads[idx];
    const correct = labelOf(step.clauseIdx);
    const options = seedRotate([...new Set(heads.map((_, idx) => labelOf(idx)))], correct);
  ...
  if (step.type === 'clauseGerundValence') {
    const gc = sentence.clusters[step.clusterIdx] || {};
    const r = valenceSpec(gc.governorWord || '', gc.transitivity, clauseCoreIn(sentence, step.clauseIdx, gc, 'karma').length > 0, true);
    return { ...r, highlight: new Set(gc.governorWordIndex != null ? [gc.governorWordIndex] : []) };
  }
...
function tutorialStepLabelBase(step, sentence) {
  ...
  switch (step.type) {
    ...
    case 'hetu': return `For ${gov}, which word is the हेतु — "due to what cause/reason" does this happen? (case: तृतीया by 2.3.23 हेतौ, or पञ्चमी by 2.3.24/25)`;
...
function tutorialOverrideNote(sentence, step, wordIndex) {
  ...
  if (SWEEP_ARRAYS.includes(step.type)) {
    ...
    if (item && item.role === 'हेतुः') return `हेतु (cause/reason) can appear in either तृतीया (2.3.23 हेतौ) or पञ्चमी (2.3.24 अकर्तर्यृणे पञ्चमी / 2.3.25 विभाषा गुणेऽस्त्रियाम्) — look at the form of ${esc(sentence.words[wordIndex])} here to tell which one it is.`;
...
function renderTutorial() {
  ...
  const suppliedOf = cl => {
    const out = (cl.elided || []).map(stripParens).filter(Boolean);
  ...
  if (step.type === 'voice') {
    ...
    if (!answered) {
      app.querySelectorAll('.opt').forEach(btn => btn.onclick = () => {
        tutorialScores.push(btn.dataset.o === c.voice ? 1 : 0);   // MCQ steps count toward verse completion too (2026-09-25; were never scored)
  ...
  if (step.type === 'kartaCase') {
    ...
    if (!answered) {
      app.querySelectorAll('.opt').forEach(btn => btn.onclick = () => {
        tutorialScores.push(acc.has(btn.dataset.o) ? 1 : 0);
  ...
  if (step.type === 'genderCheck') {
    ...
    if (!answered) {
      app.querySelectorAll('.opt').forEach(btn => btn.onclick = () => {
        tutorialScores.push(btn.dataset.o === correct ? 1 : 0);
  ...
  if (NEW_MCQ_TYPES.has(step.type)) {
    ...
    if (!answered) {
      ...
      app.querySelectorAll('.opt').forEach(btn => btn.onclick = () => { const o = spec.options[+btn.dataset.oi]; tutorialScores.push(acc.has(o) ? 1 : 0); view = { ...view, checked: true, mcqPicked: o }; renderTutorial(); });
    } else {
      document.getElementById('tutNextBtn').onclick = () => advanceTutorialStep();
    }
    wireTutorialReportArea(sentence, step, verse, renderTutorial);
...
const TUT2WALK = { 'Gita': 'Gita', 'isha-mula': 'Isha', 'kena-mula': 'Kena', 'katha-mula': 'Kathaka',
  ...
  'panchadashi': 'PD', 'atmabodha': 'AB', 'upadesha-saram': 'UPS', 'saddarshanam': 'SAD' };
